CREATE TABLE pruchase_history(product_ID int,sale_QTY int,Bill_Date date);
...
SELECT pc.Cat_Id,ph.Product_Id,DENSE_RANK() OVER (PARTITION BY Cat_Id ) AS Trending
FROM pruchase_history ph
